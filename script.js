...
async function loadStorySessions() {
    try {
        const response = await fetch('/story/sessions', {
            credentials: 'include'
        });
        
        if (!response.ok) throw new Error('Failed to load story sessions');
        
        storySessions = await response.json();
        updateStorySessionsDisplay();
...
function updateStorySessionsDisplay() {
    const sessionGrid = document.getElementById('story-sessions-grid');
    if (!sessionGrid) return;

    if (storySessions.length === 0) {
        sessionGrid.innerHTML = `
            <div class="no-sessions">
                No active story sessions. Create one to get started!
            </div>
        `;
        return;
    }

    sessionGrid.innerHTML = storySessions.map(session => `
        <div class="story-session-card" onclick="window.location.href='/story/${session.id}'">
            <div class="story-session-title">${session.title}</div>
            <div class="story-session-desc">${session.scenario.slice(0, 100)}${session.scenario.length > 100 ? '...' : ''}</div>
            <div class="story-session-characters">
                ${session.characters.map(char => `
                    <img src="${char.avatar}" alt="${char.name}" 
                         class="story-character-avatar" title="${char.name}"
